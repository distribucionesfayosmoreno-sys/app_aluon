...
export const DetailModal = ({ isOpen, onClose, items }: Props) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4 backdrop-blur-sm overflow-y-auto">
      <div
        className="relative w-full max-w-lg bg-surface text-on-surface rounded-3xl shadow-2xl overflow-hidden flex flex-col my-8"
      ...
      >
        ...
        <div className="flex-1 overflow-y-auto p-5 space-y-6">
          {items.map((item, idx) => {
            const openingSide: "LEFT" | "RIGHT" = item.bisagras ? "RIGHT" : "LEFT";
            const openingImage =
              item.doorType === "VALLA" ? null : openingImagePath(item.doorType, openingSide);

            return (
              <div
                key={`${item.doorModel}-${idx}`}
                className="border-b border-outline-variant/10 pb-5 last:border-0 last:pb-0 space-y-4"
              >
